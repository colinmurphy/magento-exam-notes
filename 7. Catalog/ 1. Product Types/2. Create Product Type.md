...
So we are going to increase the price by 25% if the ticket is platnum in our price model.
    ...
    class Colin_Catalog_Model_Product_Price extends Mage_Catalog_Model_Product_Type_Price
    {
        public function getFinalPrice($qty = null, $product)
        {
            ...
            if ($product->getTicketType() == Colin_Catalog_Model_Source_Ticket::TICKET_PLATNUM)
            {
                $price = $price * 1.5;
            }
...
- attribute groups for EAV atttributes
- How are types loaded
